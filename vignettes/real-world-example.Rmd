...
r_pivot_longer <- record(pivot_longer)
r_filter <- record(filter)
r_mutate <- record(mutate)
r_separate <- record(separate)
...
```{r}
avia_clean <- avia %>%
  r_select(1, contains("20")) %>% # select the first column and every column starting with 10
