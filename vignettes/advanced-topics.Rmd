...
(This definition is an oversimplification of the actual definition of a monad, but good enough for
our purposes.)
...
a `Monad` (in Haskell) is *something* that has three methods:
...
- and `return`. Don't be confused by the name, this has nothing to do with the `return()` we use inside functions to return a value. `return` is a function that coerces a value into a monadic value, so if you consider any object `a`, `return` takes `a` as an input and *returns* the monadic value `m a`.
...
I did not focus on `return`/`unit`.
...
In sum, monads are useful when you need values to also carry something more with them as shown here, 
a log for instance. For another example of a monad implemented as an R package, see the 
[maybe monad](https://armcn.github.io/maybe/).
