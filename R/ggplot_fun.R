  ...
  res <- tryCatch(
    withCallingHandlers(
      # The main expression to evaluate
      {
        ...
        # Then, force its evaluation by building it. This triggers lazy errors
        # without rendering the plot to a device. We discard the build data.
        ggplot2::ggplot_build(ggplot_obj)

        # If ggplot_build() succeeds, the original object is valid. Return it.
        ggplot_obj
      },
