...
`{chronicler}` provides the `record()` function, which allows you to decorate functions. These
decorated functions then work exactly the same as their undecorated counterparts, but they
...
line for now; for more details see the `Maybe Monad` vignette). The second part, tells you that there's more to it:
