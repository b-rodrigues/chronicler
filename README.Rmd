...
`{chronicler}` can capture other conditions than just errors. "Capturing" here means
failing and returning the condition. `{chronicler}` can capture:

- only errors, or
- only errors and warnings, or
- errors and warnings, but in the case of warning, still return the result (useful in cases where the warning is inoffensive), or
- errors, warnings, and messages.

It is also possible to only capture errors, errors and warnings, errors and warnings but
without or capture errors, warnings and messages using
...
# Errors and warnings, but return the result as well
r_sqrt <- record(sqrt, strict = 3)
